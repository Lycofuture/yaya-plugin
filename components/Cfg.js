...
import {Plugin} from "./Version.js";

const _cfgPath = `${Plugin._path}/plugins/${Plugin.name}/components/`
let cfg = {}

let configPath = `${Plugin._path}/plugins/${Plugin.name}/config/`
let defSetPath = `./plugins/${Plugin.name}/defSet/`
...
try {
    if (fs.existsSync(_cfgPath + 'cfg.json')) {
        ...
        cfg.gachas = getConfig('dacha', 'gacha')
    }
} catch (e) {
    // do nth
}
